...
/**
 * @brief Number of DAP packet buffers in the ring buffer.
 *
 * This configuration setting defines the number of DAP packet buffers that can be stored in the
 * ring buffer. Increasing this value can improve the throughput of the DAP communication,
 * but it also increases the memory usage.
 */
...
/// 1024 for High-speed USB HID and 512 for High-speed USB WinUSB.
    ...
    #define USBIP_DAP_PACKET_SIZE 512U // 512 for WinUSB.
#else
    #define USBIP_DAP_PACKET_SIZE 255U // 255 for USB HID
#endif
...
#if (USBIP_ENABLE_WINUSB == 1)
    usbip_send_stage2_submit_ctx(ctx, header, 0, 0);
...
void handle_dap_data_response(usbip_stage2_header *header)
{
    ...
    //     usbip_send_stage2_submit_data_ctx(ctx, header, 0, (void *)DAPDataProcessed.buf, resLength);
    //     dap_respond = 0;
    // }
    // else
    // {
...
void handle_swo_trace_response(usbip_stage2_header *header)
{
    ...
    if (kSwoTransferBusy)
    {
        ...
        printf("swo use data\r\n");
        usbip_send_stage2_submit_data_ctx(ctx, header, 0, (void *)s_swo_data_to_send, s_swo_data_num);
...
int fast_reply(uint8_t *buf, uint32_t length)
{
        ...
        buf_header->base.ep == PP_HTONL(1))
    {
        if (s_dap_respond > 0)
        {
            ...
            {
#if (USBIP_ENABLE_WINUSB == 1)
                usbip_send_stage2_submit_data_fast_ctx(ctx, (usbip_stage2_header *)buf, item->buf, item->length);
#else
                usbip_send_stage2_submit_data_fast_ctx(ctx, (usbip_stage2_header *)buf, item->buf, DAP_HANDLE_SIZE);
#endif
...
{
    int count = 0;
    if (s_data_response_mux && xSemaphoreTake(s_data_response_mux, pdMS_TO_TICKS(10)) == pdTRUE)
    {
        count = s_dap_respond;
        xSemaphoreGive(s_data_response_mux);
    }
...
{
    if (s_dap_data_out_handle == NULL || packet_size == NULL)
    {
        return NULL;
    }

    return xRingbufferReceiveUpTo(s_dap_data_out_handle, packet_size,
                                  pdMS_TO_TICKS(10), DAP_HANDLE_SIZE);
}

void dap_release_out_packet(void *packet)
{
    if (packet != NULL && s_dap_data_out_handle != NULL)
    {
        vRingbufferReturnItem(s_dap_data_out_handle, packet);
    }
}

size_t dap_get_handle_size(void)
{
    return DAP_HANDLE_SIZE;
}
...
uint8_t *dap_packet_get_data(void *packet)
{
    ...
    dap_packet_t *pkt = (dap_packet_t *)packet;
    return pkt->buf;
}
...
uint32_t dap_packet_get_length(void *packet)
{
    ...
    return pkt->length;
#else
    /* HID mode doesn't have length field, return full packet size */
    return USBIP_DAP_PACKET_SIZE;
#endif
}