...
private:
    ...
    dap_err_t flash_func_start(flash_func_t func);
    const program_target_t *get_flash_algo(uint32_t addr);

public:
    TargetFlash();
    virtual dap_err_t flash_init(const target_cfg_t *cfg) override;
    virtual dap_err_t flash_uninit(void) override;
    virtual dap_err_t flash_program_page(uint32_t adr, const uint8_t *buf, uint32_t size) override;
    virtual dap_err_t flash_erase_sector(uint32_t addr) override;
    virtual dap_err_t flash_erase_chip(void) override;
    ...
    virtual dap_err_t flash_algo_set(uint32_t addr) override;
};